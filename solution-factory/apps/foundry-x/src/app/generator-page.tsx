...
export function GeneratorPage({ user }: { user: any }) {
  ...
  const [bulkResults, setBulkResults] = useState<{ topic: string; thread: string; status: "pending" | "processing" | "completed" | "error" }[]>([]);
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `FOUNDRY_X_THREAD_${new Date().getTime()}`,
  });
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Check if it's a CSV for bulk mode OR a PDF for forensic locker
      if (file.name.endsWith('.csv') && activeTab === 'bulk') {
        Papa.parse(file, {
          header: true,
          skipEmptyLines: true,
          complete: (results) => {
            ...
            if (results.meta.fields?.length) setSelectedColumn(results.meta.fields[0]);
          },
        });
        return;
      }

      // If PDF, upload to locker
      if (file.name.endsWith('.pdf')) {
        setIsUploading(true);
        try {
          const filePath = `${user.id}/x-intel/${Date.now()}-${file.name}`;
          ...
          const { data: docRecord, error: dbError } = await supabase
            ...
            .insert({
              ...
              metadata: { status: "indexing", source: "foundry-x" }
            })
            .select()
            .single();
  ...
  const downloadBulkResults = () => {
    const csv = Papa.unparse(bulkResults.map(r => ({
      Topic: r.topic,
      Generated_Thread: r.thread
    })));
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const link = document.body.appendChild(document.createElement("a"));
    link.href = URL.createObjectURL(blob);
    link.download = `bulk_threads_${new Date().getTime()}.csv`;
    link.click();
    link.remove();
  };
  ...
  const startBulkGeneration = async () => {
    ...
    setIsLoading(true);
    setError(null);
    const initialResults = csvData.map(row => ({
      topic: row[selectedColumn],
      thread: "",
    ...
    for (let i = 0; i < initialResults.length; i++) {
      ...
      setBulkResults(prev => prev.map((res, idx) => i === idx ? { ...res, status: "processing" } : res));

      try {
        const response = await fetch("/api/generate-thread", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ topic: initialResults[i].topic, styleId: selectedStyleId }),
        });
        ...
        setBulkResults(prev => prev.map((res, idx) => i === idx ? { ...res, thread: data.thread, status: "completed" } : res));
      } catch (err) {
        setBulkResults(prev => prev.map((res, idx) => i === idx ? { ...res, status: "error" } : res));
      }
    }
    setIsLoading(false);
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center w-full flex-1 px-20 text-center py-20">
        <h1 className="text-4xl font-bold mb-4">FOUNDRY X</h1>
        <p className="mb-6">Log in to deploy your digital ghostwriting team.</p>
  ...
  return (
    <div className="flex flex-col items-center w-full p-4 md:p-8 max-w-7xl mx-auto" suppressHydrationWarning>
      <div className="flex items-center justify-between w-full mb-8">
        <div>
          <h1 className="text-4xl font-bold italic tracking-tighter">FOUNDRY X</h1>
          <p className="text-muted-foreground uppercase text-xs tracking-widest font-semibold">Viral Thread Engine</p>
        </div>
      ...
      {isQuotaExceeded && (
        <Card className="mb-8 border-primary bg-primary/5 w-full">
          <CardHeader>
            <CardTitle>API Limit Reached</CardTitle>
            <CardDescription>
              The AI model has reached its generation limit. If you are a free user, please wait 24 hours or upgrade.
            </CardDescription>
          </CardHeader>
          <CardFooter>
            <Button onClick={handleUpgrade} className="w-full">Upgrade to Pro for $29/mo</Button>
          </CardFooter>
        </Card>
      )}

      {error && <div className="w-full mb-4"><Alert title="Error" message={error} /></div>}
        ...
        {activeTab === "single" ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 w-full">
            <div className="lg:col-span-1 space-y-6">
              <form onSubmit={handleSubmit}>
                <Card>
                  <CardHeader>
                    <CardTitle>Generate a Viral Thread</CardTitle>
                    <CardDescription>Topic or URL + custom writing voice.</CardDescription>
                  </CardHeader>
                  ...
                  <CardFooter className="flex justify-between">
                    <p className="text-xs text-muted-foreground">Logged in: {user.email}</p>
                    <Button type="submit" disabled={isLoading || !topic || isQuotaExceeded}>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Upload source documents to ensure your thread is backed by facts.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-2">
                  ...
                  {documents.length === 0 && !isUploading ? (
                    <div className="text-center py-8 text-muted-foreground text-xs border-2 border-dashed rounded-md">No source docs found.</div>
                  ) : (
                    documents.map((doc) => (
            ...
            <div className="lg:col-span-2 space-y-6">
              <Card className="min-h-[600px] flex flex-col overflow-visible">
                <CardHeader className="border-b bg-muted/20 flex flex-row items-center justify-between">
                  <div>
                    <CardTitle className="text-xl">Thread Intelligence</CardTitle>
                    <CardDescription>Viral content, forensically grounded.</CardDescription>
                  </div>
                  {generatedThread && (
                    <Button variant="outline" size="sm" onClick={() => handlePrint()}>
                      <Download className="mr-2 h-4 w-4" /> Export Draft
                    </Button>
                  )}
                </CardHeader>
                <CardContent className="flex-1 p-6 flex flex-col overflow-visible">
                  {!generatedThread && !isLoading ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-12 opacity-50">
                      ...
                      <p className="text-muted-foreground max-w-sm italic text-sm">
                        Upload source materials and hit "Generate Thread" to create a truth-backed viral narrative.
                      </p>
                    </div>
                  ) : (
                    <div ref={reportRef} className="p-4 print:p-8">
                      <div className="hidden print:block mb-8 border-b-2 border-primary pb-4 text-center">
                        <h1 className="text-3xl font-bold italic tracking-tighter">FOUNDRY X</h1>
                        <p className="text-xs uppercase tracking-widest text-muted-foreground font-semibold">Verified Content Intelligence</p>
                      </div>
          ...
          <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
            <Card className="h-fit">
              <CardHeader>
                <CardTitle>Bulk Setup</CardTitle>
                <CardDescription>Upload CSV and map fields.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                {csvHeaders.length > 0 && (
                  <>
                    <div className="space-y-2">
                      <Label>2. Map 'Topic' Column</Label>
                      <select 
                        className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-sm"
                    ...
                    <div className="space-y-4">
                      <Button className="w-full" onClick={startBulkGeneration} disabled={isLoading || isQuotaExceeded}>
                        <Play className="mr-2 h-4 w-4" /> Start Bulk Generation
                      </Button>
                      
                      {bulkResults.some(r => r.status === "completed") && (
                        <Button variant="outline" className="w-full" onClick={downloadBulkResults}>
                          <FileText className="mr-2 h-4 w-4" /> Download Results (.csv)
                        </Button>
                      )}
                    </div>
                  </>
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold text-xl flex items-center"><List className="mr-2 h-5 w-5" /> Generation Queue ({bulkResults.length})</h3>
                {isLoading && <span className="text-sm text-muted-foreground flex items-center"><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Processing row {currentBulkIndex + 1}...</span>}
              </div>
              ...
              <div className="space-y-4 max-h-[800px] overflow-y-auto pr-2">
                {bulkResults.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-64 border-2 border-dashed rounded-xl text-muted-foreground bg-muted/5">
                    ...
                    <p>Upload a CSV to see the queue here.</p>
                  </div>
                ) : (
                  bulkResults.map((res, i) => (
                    <Card key={i} className={i === currentBulkIndex ? "border-primary shadow-md" : ""}>
                      <CardHeader className="py-4">
                        <div className="flex items-center justify-between">
                          <CardTitle className="text-base truncate max-w-[80%]">{res.topic}</CardTitle>
                          {res.status === "completed" && <CheckCircle2 className="text-green-500 h-5 w-5" />}
                          {res.status === "processing" && <Loader2 className="text-primary h-5 w-5 animate-spin" />}
                          {res.status === "error" && <span className="text-destructive text-xs">Error</span>}
                        </div>
                      </CardHeader>
                      {res.thread && (
                        <CardContent className="pb-4 pt-0">
                          <Textarea readOnly className="h-32 text-xs bg-muted/30" value={res.thread} />
                        </CardContent>
                      )}
                    </Card>
                  ))
