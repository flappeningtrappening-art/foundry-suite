...
export function GeneratorPage({ user }: { user: any }) {
  ...
  const [bulkResults, setBulkResults] = useState<{ address: string; listing: string; status: "pending" | "processing" | "completed" | "error" }[]>([]);
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (uploadError) {
        ...
        alert(`UPLOAD ERROR: ${uploadError.message} (${(uploadError as any).statusCode || 'No Status'})`);
        return;
      }
      ...
      const { data: docRecord, error: dbError } = await supabase
        ...
        .insert({
          ...
          metadata: { status: "indexing", source: "realty" }
        })
        .select()
        .single();
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center w-full flex-1 px-20 text-center py-20">
        <h1 className="text-4xl font-bold mb-4">FOUNDRY REALTY</h1>
        <p className="mb-6">Log in to automate your property marketing suite.</p>
  ...
  return (
    <div className="flex flex-col items-center w-full p-4 md:p-8 max-w-7xl mx-auto" suppressHydrationWarning>
      <div className="flex items-center justify-between w-full mb-8">
        <div>
          <h1 className="text-4xl font-bold italic tracking-tighter">FOUNDRY REALTY</h1>
          <p className="text-muted-foreground uppercase text-xs tracking-widest font-semibold">Forensic Inventory Description Engine</p>
        </div>
      ...
      <div className="w-full space-y-8">
        <div className="flex bg-muted p-1 rounded-lg w-fit mx-auto">
          <button onClick={() => setActiveTab("single")} className={`px-6 py-2 rounded-md text-sm font-medium transition-all ${activeTab === "single" ? "bg-background shadow-sm" : "hover:text-primary"}`}>Single Listing</button>
        ...
        {activeTab === "single" ? (
            ...
            <div className="lg:col-span-1 space-y-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex flex-col space-y-1.5">
                      ...
                      <Input id="address" placeholder="123 Maple St" value={formData.address} onChange={handleInputChange} />
                    </div>
                    <div className="flex flex-col space-y-1.5">
                      <Label htmlFor="features">Manual Features</Label>
                      <Textarea id="features" placeholder="Pool, renovated kitchen..." value={formData.features} onChange={handleInputChange} />
                    </div>
                  </CardContent>
                  <CardFooter>
                    <Button type="submit" className="w-full" disabled={isLoading || !formData.address || isQuotaExceeded}>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Upload inspection reports or disclosures to find hidden value/risks.</CardDescription>
                </CardHeader>
              ...
              <Card className="min-h-[600px] flex flex-col overflow-visible">
                <CardHeader className="border-b bg-muted/20 flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>The Truth-Grounded Property Description.</CardDescription>
                  </div>
                  {generatedListing && (
                ...
                <CardContent className="flex-1 p-6 flex flex-col overflow-visible">
                  {error && (
                    <div className="mb-6">
                      <Alert title="Generation Failed" message={error} />
                    </div>
                  )}
                  {!generatedListing && !isLoading ? (
                    <div className="flex-1 flex flex-col items-center justify-center text-center p-12 opacity-50">
                      ...
                      <p className="text-muted-foreground max-w-sm italic text-sm">
                        Upload inspection reports and hit "Generate Listing" to create a forensically-verified description.
                      </p>
                    </div>
                  ) : (
                    <div ref={reportRef} className="p-4 print:p-8">
                      ...
                      <div className="hidden print:block mb-8 border-b-2 border-primary pb-4 text-center">
                        <h1 className="text-3xl font-bold italic tracking-tighter">FOUNDRY REALTY</h1>
                        <p className="text-xs uppercase tracking-widest text-muted-foreground font-semibold">Verified Property Marketing Report</p>
                        <div className="mt-4 flex justify-between text-[10px] text-muted-foreground">
                          <span>Property: {formData.address}</span>
          ...
          /* Bulk Mode remains similar but uses the new logic */
          <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Bulk Mode logic... same as before but could eventually be upgraded too */}
